...
def generate_pdf(input_json: str, output_pdf: str) -> None:
    ...
    # Title
    elements.append(Paragraph("SAST Scan Report", style_heading))
    ...
    for page_start in range(0, total_vulns, rows_per_page):
        ...
        table_data = [["Severity", "Rule", "File", "Line", "Message"]]
        for vuln in page_vulns:
            ...
            message = vuln.get("message") or vuln.get("description") or "N/A"

            truncated_msg = truncate_description(message, max_length=300)
            table_data.append(
                [
                    severity,
                    Paragraph(rule_name, wrap_style),
                    Paragraph(file_path, wrap_style),
                    str(line_num),
                    Paragraph(truncated_msg, wrap_style),
                ]
            )
...
# Example usage
generate_pdf("gl-sast-report.json", "sast-report.pdf")
