...
echo "Building npm package for project: ${CI_PROJECT_NAME} with tag: ${VERSION_TAG}"
...
if [ "$CI_PROJECT_NAME" != "tpulse-msa-fe-root-app" ]; then
  ...
  tar -czf "${CI_PROJECT_NAME}-${VERSION_TAG}.tar.gz" -C dist .
else
  ...
  tar -czf "${CI_PROJECT_NAME}-${VERSION_TAG}.tar.gz" -C temp_dir .
  rm -rf temp_dir
fi
...
EXISTING_PACKAGE_ID=$(
  curl --silent \
    ...
    | jq -r ".[] | select(.version == \"${VERSION_TAG}\").id"
);
echo "Existing package ID for ${VERSION_TAG}: $EXISTING_PACKAGE_ID";
if [ -n "$EXISTING_PACKAGE_ID" ]; then
  echo "Deleting old package files for package ID $EXISTING_PACKAGE_ID for ${VERSION_TAG}...";
  PACKAGE_FILE_IDS=$(
    curl --silent \
      --header "JOB-TOKEN: $CI_JOB_TOKEN" \
...
echo "Uploading package with version tag: ${VERSION_TAG}"
curl --header "JOB-TOKEN: $CI_JOB_TOKEN" \
     --upload-file "${CI_PROJECT_NAME}-${VERSION_TAG}.tar.gz" \
     "https://gitlab.com/api/v4/projects/${CI_PROJECT_ID}/packages/generic/${CI_PROJECT_NAME}/${VERSION_TAG}/${CI_PROJECT_NAME}-${VERSION_TAG}.tar.gz"

# 4) Handle the "latest" tag package only if CI_COMMIT_TAG is present
if [ -n "$CI_COMMIT_TAG" ]; then
  echo "Handling 'latest' tag package..."
  ...
  cp "${CI_PROJECT_NAME}-${VERSION_TAG}.tar.gz" "${CI_PROJECT_NAME}-latest.tar.gz"
  curl --header "JOB-TOKEN: $CI_JOB_TOKEN" \
  ...
  echo "'Latest' tag package uploaded successfully!"
else
  echo "CI_COMMIT_TAG is not present, skipping 'latest' tag handling."
fi
