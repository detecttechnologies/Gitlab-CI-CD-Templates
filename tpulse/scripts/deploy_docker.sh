...
if [ -z "$DEPLOY_ENVIRONMENT" ]; then
    ...
    python3 -c "$(curl -fsSL https://github.com/detecttechnologies.com/Gitlab-CI-CD-Templates/raw/dashboard-ci/tpulse/scripts/push_ecr.py)" "${DEPLOY_CONFIG}" $TAG

    python3 -c "$(curl -fsSL https://github.com/detecttechnologies.com/Gitlab-CI-CD-Templates/raw/dashboard-ci/tpulse/scripts/deploy_ecs.py)" "${DEPLOY_CONFIG}" $TAG
fi
